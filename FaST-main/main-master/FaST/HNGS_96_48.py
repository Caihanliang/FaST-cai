...
DATA_NAME = 'HNGS'
num_nodes = 161  # 湖南高速公路 161 个收费站点
INPUT_LEN = 96   # 输入长度：96 个时间步（24 小时）
OUTPUT_LEN = 48  # 预测长度：48 个时间步（12 小时）
NUM_EPOCHS = 50
BATCH_SIZE = 32  # 由于节点数较少，可以适当减小 batch size
...
MODEL_PARAM = {
    ...
    "layers": 3,           # 保持 3 层
    "num_experts": 8,      # 8 个专家
    "daily_steps": 96,     # 每天 96 个 15 分钟间隔
    "weekly_days": 7,      # 每周 7 天
    "hidden_dim": 64,      # 隐藏层维度 64
    "num_agent": 32,       # 32 个代理节点
}

############################## General Configuration ##############################
CFG = EasyDict()
# General settings
CFG.DESCRIPTION = "FaST for Hunan Highway Traffic Flow Forecasting"
CFG["fp16"] = True  # 使用混合精度训练
...
CFG.MODEL.FORWARD_FEATURES = [0, 1, 2]  # 使用流量、TOD、DOW 三个特征
